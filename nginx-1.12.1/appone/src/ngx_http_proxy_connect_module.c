...
ngx_http_proxy_connect_upstream_handler(ngx_event_t *ev)
{
    ...
    ngx_log_debug3(NGX_LOG_DEBUG_HTTP, c->log, 0,
                   "http proxy_connect upstream handler: \"%V|%V:%V\"",
                   &r->connect_proto, &r->connect_host, &r->connect_port);
    ...
    ngx_http_proxy_connect_upstream_t *u)
{
    ...
    if (ngx_strncmp(r->connect_proto.data, "udp", 3) == 0)
    {
	pc->type = SOCK_DGRAM;
    }
...
ngx_http_proxy_connect_post_read_handler(ngx_http_request_t *r)
{
	...
	rc=ngx_http_appcore_get_appid_from_cookie(r, &appid);
	if(rc != NGX_OK)
	{
            ngx_log_error(NGX_LOG_INFO, r->connection->log, 0, "get appid from cookie error");
            return NGX_ERROR;
	}
	rc=ngx_http_appcore_get_servername(r, &servername);
	if(rc != NGX_OK)
	{
            ngx_log_error(NGX_LOG_INFO, r->connection->log, 0, "get servername from cookie error");
            return NGX_ERROR;
	}
#endif
