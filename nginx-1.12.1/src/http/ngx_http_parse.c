...
ngx_http_parse_request_line(ngx_http_request_t *r, ngx_buf_t *b)
{
    ...
    enum {
	    ...
	    sw_spaces_before_connect_proto,
        sw_connect_proto_start,
        sw_connect_proto,
        sw_connect_proto_end,
        ...
        case sw_method:
            if (ch == ' ') {
                ...
                if (r->method == NGX_HTTP_CONNECT) {
                    state = sw_spaces_before_connect_proto;
                }
#endif
...
#if (NGX_HTTP_PROXY_CONNECT)
        case sw_spaces_before_connect_proto:
            ...
            /* fall through */
        case sw_connect_proto_start:
            c = (u_char) (ch | 0x20);
            if (c >= 'a' && c <= 'z')
            {
                r->connect_proto_start = p;
                state = sw_connect_proto;
                break;
            }

            /* fall through */
        case sw_connect_proto:
            c = (u_char) (ch | 0x20);
            if (c >= 'a' && c <= 'z'){
                break;
            }

        case sw_connect_proto_end:
            r->connect_proto_end = p;
            if( ch == ':'){
